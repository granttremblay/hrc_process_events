...
* FILE NAME: hot_pixel_functions.c
...
* DESCRIPTION: This file contains several functions used by 
  ...
  The modules in this file are:
     ...
     update_hot_pixel_list()
     cleanup_hot_pixel_data()
     check_for_hot_pixels()
     load_hot_pixel_files()
     open_hot_pixel_file()
     map_hot_pixel_column()
     load_hot_pixel_data()
...
* FUNCTION NAME: update_hot_pixel_list()  

* DESCRIPTION:  The routine update_hot_pixel_list() is called on by
  load_hot_pixel_files() to insert hot spot (bad pixel) entries into the
   ...
   BAD_PIX_P_T* head_p) /* I/O - hot pixel list head */
{
   ...
   while (!found)
   {
      ...
      else if ((temp_p->x < curr_p->x) || 
               ((temp_p->x == curr_p->x) && (temp_p->y < curr_p->y))) 
      { 
...
* FUNCTION NAME: cleanup_hot_pixel_data() 
 
* DESCRIPTION:  The routine cleanup_hot_pixel_data() is called by 
...
void cleanup_hot_pixel_data(BAD_PIX_A_T bad_pix_list) /* I/O */  
{
   int rr;
...
* FUNCTION NAMES: load_hot_pixel_files() 

* DESCRIPTION:  The routine load_hot_pixel_files() is called by 
   ...
   BAD_PIX_P_T*  hotpix_list_p) /* I/O - hot pix list        */
{
   ...
   while ((bad_hk.file = stk_read_next(bad_hk.stack)) != NULL)
   {
      ...
      if (opened |= (open_hot_pixel_file(&bad_hk)))
      {
         load_hot_pixel_data(&bad_hk, hotpix_list_p);
...
* FUNCTION NAME: open_hot_pixel_file()

* DESCRIPTION
 
  The routine open_hot_pixel_file is called from within a level one hrc 
   ...
   BAD_PIX_SETUP_P_T  hk_p) /* I/O - bad pixel housekeeping info */ 
{
      ...
      (strcmp(hk_p->file, "\0") != 0))
   {
      if (dmDatasetAccess(hk_p->file, "R") == dmTRUE)
      {
         ...
         /* open file */
         hk_p->dataset = dmDatasetOpen(hk_p->file);
         hk_p->extension = dmBlockOpen(hk_p->dataset, "BADPIX");
               ...
               sizeof(dmDescriptor*))) != NULL))
         {
            ...
            for (ii = 0; ii < hk_p->num_cols; ii++)
            {
               ...
               hk_p->mapping[ii] = map_hot_pixel_column(name);
            } /* end for */
...
* FUNCTION NAME: map_hot_pixel_column()
...
short map_hot_pixel_column (
   char name[])     /* I - number of columns in event */
{
    short mapping;
    short pos;
 
    if (ds_strcmp_cis(name, BAD_RAWX_COL) == 0)
    {
       mapping = BAD_RAWX;
    }
    else if (ds_strcmp_cis(name, BAD_RAWY_COL) == 0)
    {
       mapping = BAD_RAWY;
    }
    else if (ds_strcmp_cis(name, BAD_CHIPID_COL) == 0)
    {
       mapping = BAD_CHIPID;
    }
    else
...
* FUNCTION NAME: load_hot_pixel_data()
 
* DESCRIPTION:
    This function takes in a mapping value (produced in the routine 
    map_hot_pixel_column) and reads the appropriate column from the 
    ...
    BAD_PIX_P_T*      hotpix_list_p) /* O - hot pixel list pointer     */
{
           ...
           (hk_p->row_check != dmNOMOREROWS))
    {
       ...
       for (col = 0; col < hk_p->num_cols; col++)
       {
          ...
          {
             case BAD_RAWX:
                new_entry->x = dmGetScalar_s(hk_p->desc[col]);  
             break;
       
             case BAD_RAWY:
                new_entry->y = dmGetScalar_s(hk_p->desc[col]);  
             break;
       ...
       update_hot_pixel_list(new_entry, hotpix_list_p);
...
* FUNCTION NAME: check_for_hot_pixels() 
 
* DESCRIPTION: The routine check_for_hot_pixels() is called to determine 
  ...
  linked list and checks the hot spot coordinates against the event's raw 
...
* NOTES: 
  ...
  The routine converts the raw coordinates of an event from doubles to 
  ...
  written to the output qpoe file. 
  ...
  load_hot_pixel_files routine to create the linked list ensures that 
  these conditions are met.
 
*************************************************************************/

void check_for_hot_pixels(
   BAD_PIX_A_T   hot_pixel_p, /* I - pointer to a list of hot pixels    */
   EVENT_REC_P_T evt_p)       /* I/O - event record structure pointer   */
{
   ...
   if ((evt_p->chipid > -1) && (evt_p->chipid < 3)) /* range 0-2 */
   {
      curr_p = hot_pixel_p[evt_p->chipid]; 
   } 
   else
   {
   ...
   pix_double_to_short(evt_p->rawpos[HDET_PLANE_X], FALSE, &raw_x);
   pix_double_to_short(evt_p->rawpos[HDET_PLANE_Y], FALSE, &raw_y);
